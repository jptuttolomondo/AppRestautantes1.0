import {
  ...
  LIMPIAR_ESTADOS,
  CLEAR_ALL_STATES
} from "../actions/mesas.actions.jsx";
...
const mesasReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case INCREMENTAR_CANTIDAD:
      return {...state,cantidadItem: action.payload,producto: state.productoItem,subtotal: state.subtotalItem};
    ...
    case CALCULAR_SUBTOTAL:
      return {...state,subtotalItem: Number(state.cantidadItem) * Number(state.precioItem)};
